...
int main(int argc, char **argv) {
    ...
    BatchNorm * bat  = new BatchNorm(conv);
    network.push_back(bat);
    printf("bat out size %d x %d x %d x %d\n", bat->out_dim_size(0),
                                                bat->out_dim_size(1),
                                                bat->out_dim_size(2),
                                                bat->out_dim_size(3));

    ReLU * relu = new ReLU(bat);
    ...
    MaxPooling * pool = new MaxPooling(p_w, p_h, p_stride, relu, 0);
    ...
    Affine * fc = new Affine(C, reg, flatten, 0);
    network.push_back(fc);
    printf("fc out size %d x %d\n", fc->out_dim_size(0),
                                    fc->out_dim_size(1));

    SoftMax * softm = new SoftMax(fc, 0);
    ...
    //Schedule for test
    bat->forward.compute_root().parallel(bat->x).vectorize(bat->w, 4);
    ...
    conv->f_param_grads[0].update().parallel(conv->n);
    conv->f_param_grads[1].compute_root();

    //Schedule for test
    //bat->f_param_grads[0].compute_root().parallel(bat->x);
    //bat->f_param_grads[1].compute_root().parallel(bat->x);
    //bat->mult_2.compute_root();
    //bat->mult_2.trace_stores();
    bat->f_in_grad.compute_root().parallel(bat->x).vectorize(bat->w);
    ...
    init_gaussian(conv->params[0], 0.0001 * 0.0, 0.1 * 1.0, rd);
    init_gaussian(fc->params[0], 0.0001 * 0.0, 0.1 * 1.0, rd);
    init_gaussian(bat->params[0], 0.0001 * 0.0, 0.1 * 1.0, rd);  

    // Initialize the biases in the network
    init_constant(conv->params_cache[1], 0.0);
    init_constant(fc->params_cache[1], 0.0);
    init_constant(bat->params_cache[1], 0.0);

    // Initialize weights in the network
    init_gaussian(conv->params_cache[0], 0.0, 0.0, rd);
    init_gaussian(fc->params_cache[0], 0.0, 0.0, rd);
    init_gaussian(bat->params_cache[0], 0.0, 0.0, rd);
    ...
    for (int it = 0; it < num_iterartions; it++) {
        ...
        train.realize({loss, scores, conv->param_grads[0], conv->param_grads[1], bat->param_grads[0], bat->param_grads[1],
        ...
        printf("Iteration %d : loss %f, time %f, learning rate %f\n", it, loss(0), time, lr);
